...
class WriteReviewViewModel : ViewModel() {
    ...
    fun submitReview(foodId: String, orderId: String, onSuccess: () -> Unit, onError: () -> Unit) {
        if (_rating.value == 0) return

        viewModelScope.launch {
            _isLoading.value = true
            val success = repository.submitReview(orderId, foodId, _rating.value, _comment.value)
